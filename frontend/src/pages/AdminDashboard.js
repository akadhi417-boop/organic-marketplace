        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-[#E5E0D8] p-6">
              ...
              <p className="font-heading text-4xl font-bold text-moss" data-testid="stat-revenue">₹{Number(stats?.total_revenue ?? 0).toFixed(2) || '0.00'}</p>
            </div>
          </div>
              ...
              {orders.slice(0, 10).map((order) => (
                <div key={order.id} className="border border-[#E5E0D8] rounded-xl p-6" data-testid={`order-${order.id}`}>
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div>
                      <p className="font-heading text-2xl font-bold text-moss">₹{Number(order?.total_amount ?? 0).toFixed(2)}</p>
                    </div>
                    <div>
                      <select
                        value={order.status}
